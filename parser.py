...
class Parser:
    ...
    def match(self, expected_type: TokenType) -> Token:
        """Valida o tipo, escreve no XML e avança. Único ponto de escrita."""
        token = self.peek()
    ...
    def match_keyword(self, *valid_types: TokenType) -> Token:
        """
        Para tokens cujo tipo pode ser um de vários keywords conhecidos
        (ex.: static|field, constructor|function|method).
        Valida contra o conjunto, escreve e avança.
        """
        token = self.peek()
        if token and token.type in valid_types:
            self.write_token(token)
            self.current += 1
            return token
        expected = " | ".join(t.name for t in valid_types)
        lexeme = token.lexeme if token else "EOF"
        line   = token.line   if token else "?"
        raise SyntaxError(f"Linha {line}: Esperado {expected}, obtido '{lexeme}'")

    def match_type(self) -> Token:
        """Casamento para tipos: int, char, boolean ou className (IDENT)."""
        token = self.peek()
        if token and token.type in (TokenType.INT, TokenType.CHAR, TokenType.BOOLEAN, TokenType.IDENT):
            self.write_token(token)
            self.current += 1
            return token
        lexeme = token.lexeme if token else "EOF"
        line   = token.line   if token else "?"
        raise SyntaxError(f"Linha {line}: Tipo esperado, obtido '{lexeme}'")
    ...
    def parse_class_var_dec(self):
        ...
        self.match_keyword(TokenType.STATIC, TokenType.FIELD)  # static | field
    ...
    def parse_statements(self):
        ...
        stmt_types = (TokenType.LET, TokenType.IF, TokenType.WHILE, TokenType.DO, TokenType.RETURN)
    ...
    def parse_expression(self):
        ...
        ops = (
            ...
            TokenType.AND,  TokenType.OR,    TokenType.LT,       TokenType.GT,
            TokenType.EQ,
        )
        while self.peek() and self.peek().type in ops:
            self.match_keyword(*ops)  # operador: valida + escreve
            self.parse_term()
    ...
    def parse_term(self):
        ...
        if token.type == TokenType.NUMBER:
            ...
            self.match_keyword(TokenType.TRUE, TokenType.FALSE, TokenType.NULL, TokenType.THIS)
            ...
            self.match_keyword(TokenType.MINUS, TokenType.NOT)
            self.parse_term()

        elif token.type == TokenType.IDENT:
            next_t = self.peek(1)
            if next_t and next_t.type == TokenType.LBRACKET:       # array: varName[expr]
                self.match(TokenType.IDENT)
                self.match(TokenType.LBRACKET)
                self.parse_expression()
                self.match(TokenType.RBRACKET)
            elif next_t and next_t.type in (TokenType.LPAREN, TokenType.DOT):  # chamada
                self.parse_subroutine_call_parts()
    ...
    def parse_subroutine_call_parts(self):
        """Lógica para chamadas de subrotina sem abrir tag própria."""
        self.match(TokenType.IDENT)  # name ou className/varName
